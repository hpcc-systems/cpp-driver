...
void ControlConnection::on_query_meta_all(ControlConnection* control_connection,
  ...
  if (session->config().use_schema()) {
    ...
    if (control_connection->protocol_version_ >= 4) {
      session->cluster_meta().update_functions(static_cast<ResultResponse*>(responses[6]));
      session->cluster_meta().update_aggregates(static_cast<ResultResponse*>(responses[7]));
    }
...
void ControlConnection::on_refresh_function(ControlConnection* control_connection,
  ...
  if (data.is_aggregate) {
    control_connection->session_->cluster_meta().update_aggregates(result);
  } else {
    control_connection->session_->cluster_meta().update_functions(result);
  }
}
